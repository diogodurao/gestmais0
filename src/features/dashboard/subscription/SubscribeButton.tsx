...
export function SubscribeButton({ buildingId, quantity, pricePerUnit, dictionary }: SubscribeButtonProps) {
    ...
    return (
        <div className="space-y-3 w-full max-w-sm">
            <div className="flex justify-between items-center bg-slate-50 p-3 tech-border">
                <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{dictionary.subscription.totalCalculation}</p>
            ...
            >
                {isPending ? dictionary.subscription.redirecting : dictionary.subscription.commitSubscription}
            </Button>
            <p className="text-[9px] text-center text-slate-400 uppercase font-bold tracking-tighter">{dictionary.subscription.secureTransaction}</p>
        </div>
    )
}
...
export function SyncSubscriptionButton({ buildingId, dictionary }: SyncSubscriptionButtonProps) {
    const [isPending, startTransition] = useTransition()
        ...
        <div className="p-3 bg-blue-50/50 tech-border border-blue-100 max-w-sm">
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                    <p className="text-[11px] font-bold text-blue-900 uppercase">{dictionary.subscription.awaitingSync}</p>
                    ...
                    >
                        {isPending ? dictionary.subscription.syncing : dictionary.subscription.syncPayment}
                    </Button>
                    {result && (
                        <div className={`mt-2 flex items-center gap-1 text-[10px] font-bold uppercase ${result.status === 'active' ? 'text-green-700' : 'text-amber-700'}`}>
                            {result.status === 'active' ? (
                                <><CheckCircle className="w-3 h-3" /> {dictionary.subscription.stateActive}</>
                            ) : (
                                <><AlertCircle className="w-3 h-3" /> {result.message || dictionary.subscription.noSession}</>
                            )}
                        </div>
                    )}
                </div>
