    ...
    <div className="space-y-6">
      <div className="bg-info-light border border-info rounded-md p-4">
        ...
        <p className="text-label text-info">
          Como gestor, também deve identificar qual é a sua fração no condomínio. Esta
          ação irá associá-lo como proprietário.
        </p>
      </div>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isSelected
                  ? "bg-primary border-primary text-white shadow-md ring-2 ring-primary-light"
      ...
      {error && (
        <p className="text-label text-error font-semibold text-center">{error}</p>
      )}

      {apartments.length === 0 && (
        <div className="text-center p-8 border-2 border-dashed border-gray-200 rounded-lg">
          <p className="text-body text-gray-400">Não existem frações criadas.</p>
        </div>
      )}
    </div>
  )
